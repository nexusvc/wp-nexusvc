...
class ZipcodeField extends \GF_Field_Text {
    ...
    public function get_field_input( $form, $value = '', $entry = null ) {
        ...
        return sprintf( "<div class='ginput_container ginput_container_zipcode_lookup'><input name='input_%d' id='%s' type='{$html_input_type}' value='%s' maxlength='5' class='%s' pattern='%s' {$tabindex} {$placeholder_attribute} {$required_attribute} {$invalid_attribute} {$aria_describedby} %s/>{$instruction_div}</div>", $id, $field_id, esc_attr( $value ), esc_attr( $class ), '(\d{5}([\-]\d{4})?)', $disabled_text );
